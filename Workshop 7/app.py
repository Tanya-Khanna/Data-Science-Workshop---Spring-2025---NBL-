# ----------------------------------------------------------------------------------------
# 🧠📺 YT Buddy: AI-Powered YouTube Video Analyzer
...
# ----------------------------------------------------------------------------------------
# 📦 Import necessary modules for handling prompts, using a language model,
...
# This line imports two functions — `urlparse` and `parse_qs` — from Python’s built-in `urllib.parse` module.
...
# ----------------------------------------------------------------------------------------
# ⚙️ Page configuration for Streamlit app
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# 🤖 Initialize the Ollama LLM with the LLaMA 3.2B model and optimized generation parameters
# ----------------------------------------------------------------------------------------
...
llm = Ollama(
    ...
    num_ctx=2048,         # Sets the context window, i.e., the number of tokens the model can consider at once.
                          # 2048 is a good fit for LLaMA 3B models—enough to handle decent-sized transcripts.
...
# ----------------------------------------------------------------------------------------
# 🧠 Optimized prompt template for the instruct-tuned LLaMA model
# ----------------------------------------------------------------------------------------

# This block creates a prompt template using LangChain’s ChatPromptTemplate.
...
# ----------------------------------------------------------------------------------------
# 🔗 Create the processing chain by combining the prompt and the model
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# 🗂️ Initialize session state for storing transcript content
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# 🧹 clean_transcript(): Clean and format transcript text for model input
# ----------------------------------------------------------------------------------------
...
def clean_transcript(text):
    ...
    # -------------------------------
    # text.split()           → splits the string into a list of words (default splits on any whitespace)
    # ' '.join(...)          → joins those words back into a single string with one space between each
    # This effectively removes extra spaces, tabs, or newlines.

    text = ' '.join(text.split())

    # -------------------------------
    # 2️⃣ Remove non-verbal markers
    # -------------------------------
    # .replace(old, new)     → replaces occurrences of `old` substring with `new` (in this case, empty string '')
...
# ----------------------------------------------------------------------------------------
# 📄 get_transcript_content(): Extract and clean transcript from a YouTube URL
# ----------------------------------------------------------------------------------------
...
def get_transcript_content(url):
    ...
    try:
        ...
        # query_params.get('v', [None]) → returns a list; [0] accesses the first value or None if missing
        video_id = query_params.get('v', [None])[0]

        # -------------------------------
        # 2️⃣ Validate the video ID
        # -------------------------------
        if not video_id:
...
# ----------------------------------------------------------------------------------------
# 💡 SUGGESTED_QUESTIONS: Predefined queries for quick interaction
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
# 🏠 main_page(): Main Streamlit UI logic for the app
# ----------------------------------------------------------------------------------------
...
def main_page():
    ...
    # -------------------------------------
    # 1️⃣ Show app title and user guidance
    # -------------------------------------
    st.markdown("""
    ...
    **Tips:**
    ...
    # -------------------------------------
    # 2️⃣ Input field for YouTube URL
    # -------------------------------------
    url = st.text_input(
    ...
    # -------------------------------------
    # 3️⃣ Create two columns for UI layout
    # -------------------------------------
    ...
    # -------------------------------------
    # 4️⃣ Load transcript on button click
    # -------------------------------------
    if clicked:
    ...
    # -------------------------------------
    # 5️⃣ Show transcript and Q&A interface
    # -------------------------------------
...
# ✅ Ensure certain code only runs when this file is the entry point
# ✅ Prevent code from executing during imports (e.g., in testing or other scripts)
...
# 2️⃣ Step 2: (Optional but recommended) Create and Activate a Virtual Environment
# --------------------------------------------------------------------------------
